// This code belongs in static/js/social-media-automation.js
...
class SocialMediaAutomation {
    ...
    setupEventListeners() {
        ...
        // Assuming you might add this button later
    ...
    displayContentResults(recommendations) {
        ...
        resultsDiv.innerHTML = recommendations.map(rec => `
            <div class="bg-white border border-gray-200 rounded-lg p-4 mb-4 shadow-sm">
                <p class="text-gray-800">${rec.content.replace(/\n/g, '  
')}</p>
                <div class="border-t mt-4 pt-4">
                    <strong class="text-sm text-gray-600">SEO Score: ${rec.seo_score || 'N/A'}</strong>
                    <ul class="list-disc list-inside text-sm text-gray-500 mt-2">${(rec.seo_recommendations || []).map(r => `<li>${r}</li>`).join('')}</ul>
                </div>
                <button class="create-ab-test-btn mt-4 bg-indigo-500 text-white px-3 py-1 text-sm rounded-md hover:bg-indigo-600" data-content='${JSON.stringify(rec)}'>Create A/B Test</button>
            </div>
        `).join('');
    }
